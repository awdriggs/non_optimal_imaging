...
def send_image_to_server(image_path: Path, camera_id: str):
    """
    Sends an image file to the central server.
    Fails gracefully with a printed message if the server is unreachable.

    Args:
        image_path (Path): The full path to the image file to send.
        camera_id (str): The unique ID of this camera (e.g., "no00").
    """
